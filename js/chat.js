...
var appId = '2dK8teH6V9RBvbY40cJl9kik-gzGzoHsz';
var appKey = 'HvLKR7jLbq1O32GbzEy8OfFK';
...
var roomId = '5e6b486f90aef5aa84b2727b';
...
function showMsg(message, isBefore) {
  ...
  AV.Promise.resolve()
    ...
    .then(function(from) {
        ...
        if (message instanceof AV.FileMessage) {
            showLog(
            '（' +
                formatTime(message.timestamp) +
                '）  ' +
        ...
        if (message instanceof AV.ImageMessage) {
            showLog(
            '（' +
                formatTime(message.timestamp) +
                '）  ' +
...
function sendImage(){
    ...
    fileUploadControl = document.getElementById('image');
